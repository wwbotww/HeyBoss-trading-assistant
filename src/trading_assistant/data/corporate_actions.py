...
class CorporateActionRepository:
    ...
    def __init__(self, path: Path) -> None:
        ...
        self._catalog_path = (
            self._path.with_name(self._path.name.removesuffix("-actions"))
            if self._path.name.endswith("-actions")
            else self._path
        )
        self._catalog_path.mkdir(parents=True, exist_ok=True)
        if not (self._catalog_path / ".catalog.lock").exists():
            with catalog_lock(self._catalog_path, exclusive=True):
                pass
    ...
    def read(self, instrument_id: str) -> CorporateActions:
        ...
        with catalog_lock(self._catalog_path, exclusive=False):
    ...
    def replace_range(
        ...
        *,
        ...
        with catalog_lock(self._catalog_path, exclusive=True):
            ...
            dividends = {
                item.ex_date: item
                for item in existing.dividends
                if not start <= item.ex_date <= end
            }
